...
md`<h2>結論</h2>
  <ul>
    <li>此調查發現，多數參與者是來自工作室的藝術從業者</li>
    <li>這些藝術家主要透過減少使用包裝材料來降低他們的碳足跡</li>
    <li>相比之下，美術館的藝術家不僅減少了包裝的使用，還積極在工作場所節能，但與其他地方的藝術家相比，他們較少使用再生材料作為降低碳排放的策略</li>
  </ul>
</h3>`
)}
...
d3
  ...
  //.range(d3.schemeTableau10) // 更換配色方案
  .range(["#3E5F8A", "#F2A2E8", "#9D5C63", "#F9BF45", "#58A55C", "#7272A7"])
  .unknown("#CCCCCC")
)}
...
htl.html`<style>
.tooltip {
  padding: 10px 15px; /* 更改填充大小 */
  color: #f0f0f0; /* 更淺的文字顏色 */
  border-radius: 4px; /* 更小的邊角半徑 */
  border: 1px solid rgba(255,255,255,0.3); /* 更細且更透明的邊框 */
  box-shadow: 0 2px 6px 0 rgba(0,0,0,0.3); /* 更大且更深的陰影 */
  pointer-events: auto; /* 啟用滑鼠事件 */
  transform: translate(-60%, -120%); /* 調整轉換位置 */
  font-family: "Arial", sans-serif; /* 改用Arial字體 */
  background: rgba(30,20,40,0.7); /* 更深且稍微透明的背景顏色 */
  transition: 0.3s opacity ease-in, 0.2s border-color ease-in; /* 改變過渡效果和時間 */
}
</style>
`
)}
